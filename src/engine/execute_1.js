...
const execute = function (sequencer, thread) {
    ...
    // 增加对 disbaled 块线程的判断
    const blockObj = blockContainer.getBlock(currentBlockId);
    if (blockContainer.isDisabled(blockObj)) {
        return;
    }
...
// TODO: excute.js 被重构后，有几个定制需求要补上去
// Skip through the block (hat with no predicate).
// 需求1: 帽子块也要上报执行事件
// runtime.emit(runtime.constructor.BLOCK_SCRIPT_RAN, Object.assign({
//     blockId: currentBlockId, opcode: opcode
// }, {/*参数暂时缺省*/ }));
// thread.requestScriptGlowInFrame = true;

// 需求2: promise 块运行完毕需正常触发 onEnd 事件的
// if (thread.status === Thread.STATUS_RUNNING) {
//     // Primitive returned a promise; automatically yield thread.
//     thread.status = Thread.STATUS_PROMISE_WAIT;
